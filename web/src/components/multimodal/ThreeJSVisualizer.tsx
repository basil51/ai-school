...
export default function ThreeJSVisualizer({ 
  ...
  const createGeometry = useCallback((geoConfig: any = {}) => {
  ...
  const createMolecule = useCallback((chemConfig: any = {}) => {
  ...
  const createPhysicsSimulation = useCallback((physicsConfig: any = {}) => {
  ...
  const createMathFunction = useCallback((mathConfig: any = {}) => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      try {
        ...
        console.error('Error creating math function:', e);
        y = 0;
      }
  ...
  }, [type, config, width, height, createGeometry, createMolecule, createPhysicsSimulation, createMathFunction]);
